...
export async function GET(
  ...
  try {
    ...
    const formattedPredictions = predictions.map((prediction) => {
      ...
      console.log(
        "match",
        prediction.poll.options.filter((option) => option.isCorrect == true)
      );
      return {
        id: prediction.id,
        teams: `${match.homeTeam.name} vs ${match.awayTeam.name}`,
        date: match.date,
        prediction: prediction.poll.options.filter(
          (option) => option.id === prediction.optionId
        )[0].text,
        actual:
          prediction.poll.options.filter(
            (option) => option.isCorrect == true
          )[0]?.text || "PENDING",
        isCorrect:
          prediction.poll.options.filter(
            (option) => option.isCorrect == true
          )[0]?.id === prediction.optionId,
        type: prediction.poll.type,
        points:
          prediction.poll.options.filter(
            (option) => option.isCorrect == true
          )[0]?.id === prediction.optionId
            ? prediction.poll.points ?? 10
            : 0,
